...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item Compute gain filters only at points where the stimulus is significantly away form the mean
   \item Check if Weber's Law is being followed using Carlotta's step reponses data
   \item Fit DA model to data with tau set to zero
   \item Use filtfilt instead of filter to filter signals
   \item Filter ORN firing rates with a 30ms box to remove wiggles that are just noise
