...
function [R, a, w_plus, w_minus, e0] = bacteriaModelX(S,p)

% work with matrices too
if size(S,2) > 1
	R = NaN*S; a = NaN*S;  w_plus = NaN*S; w_minus = NaN*S; e0 = NaN*S; 
	for i = 1:size(S,2)
		[R(:,i),a(:,i),w_plus(:,i), w_minus(:,i), e0(:,i)] = bacteriaModelX(S(:,i),p);
	end
	return
end
...
for i = 2:length(S_)
	...
	w_plus(i) = p.w0*(num/denom)*exp(-e0_(i-1));

	denom = 1 + S_(i-1)/p.K_1; 
	w_minus(i) = p.w0*(num/denom);
