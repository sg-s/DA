...
%% Firing Rate
% First, we perform this analysis with the firing rate response of the ab3A neuron to ethyl acetate odorant. 
...
% We now repeat the analysis with the LFP.
...
r2_plot_data = fastGainControlAnalysis(ax(3:end),orn_data,'recompute_DA_fit',false,'data','LFP');
...
%% Light Activation
% Now we repeat the analysis on a dataset where we use light to excite the ORN, with no odorant whatsoever. 
...
% if ~exist('ld','var')
% 	p = '/local-data/DA-paper/Chrimson/nat-stim-mimic/';
% 	ld = raw2ORNData(p,'led_power_func',led_power_func,'use_led',true,'filter_LFP',false);

% 	uts = false(length(ld(1,1).stimulus),1);
% 	uts(10e3:end-10e3) = true;
% 	for i = 1:size(ld,1)
% 		for j = 1:size(ld,2)
% 			if ld(i,j).n_trials > 0
% 				disp([i j])
% 				ld(i,j).use_this_segment = uts;
% 				ld(i,j) = backOutFilters(ld(i,j));
% 			end
% 		end
% 	end
% end
...
% % show stimulus
% t = (1:length(example_data.stimulus))*1e-3;
% plot(ax(1),t,nanmean([example_data.stimulus],2),'k')
% xlabel(ax(1),'Time (s)')
% ylabel(ax(1),'Stimulus (\uW)')
% set(ax(1),'XLim',[0 70])

% % show response
% plot(ax(2),t,nanmean([example_data.firing_rate],2),'Color','k')
% xlabel(ax(2),'Time (s)')
% ylabel(ax(2),'Firing Rate (Hz)')
% set(ax(2),'XLim',[0 70])
