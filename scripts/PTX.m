...
%% Pertussis Toxin to interfere with G-protein machinery 
...
[alldata(1).PID, alldata(1).LFP, alldata(1).fA, alldata(1).paradigm, alldata(1).orn, alldata(1).fly, alldata(1).AllControlParadigms, alldata(1).paradigm_hashes] = consolidateData('/local-data/DA-paper/ptx',1);
...
%%
% It looks like the PTX mutant is more sensitive (so the LFP deflections are bigger), but also that a slow timescale in the CS (the rate of adaptation to the overall step) becomes much slower (i.e., see that the lowest dose elicits a almost perfect adaptation in the CS, but there is hardly any step adaptation in the PTX mutant). 
