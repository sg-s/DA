...
input_contrast_change_K1p = NaN(width(reshaped_LFP),1);
input_contrast_change_K2K1p = NaN(width(reshaped_LFP),1);
input_contrast_change_K2p = NaN(width(reshaped_LFP),1);
input_contrast_change_K3p = NaN(width(reshaped_LFP),1);
gain_change_K1p = NaN(width(reshaped_LFP),1);
gain_change_K2K1p = NaN(width(reshaped_LFP),1);
gain_change_K2p = NaN(width(reshaped_LFP),1);
gain_change_K3p = NaN(width(reshaped_LFP),1);
...
for i = 1:width(reshaped_LFP)
	...
	if r2_K2p(i) > min_r2
		...
		input_contrast_change_K2p(i) = std(reshaped_LFP(6e3:9e3,i))/std(reshaped_LFP(1e3:4e3,i));
	end
