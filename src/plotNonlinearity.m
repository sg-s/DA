...
for i = 1:length(groups)
	...
	elseif strcmp(plot_options.data_bin_type,'dots')
		...
		elseif strcmp(plot_options.plot_type,'mean')
			plot_handles(i).line = plot(plot_here,nanmean(this_pred(1:plot_options.nbins:end,:),2),nanmean(this_resp(1:plot_options.nbins:end,:),2),'Color',c(i,:),'Marker','.','LineStyle','none');
			if plot_options.showr2
				r2 = rsquare(nanmean(this_pred,2),nanmean(this_resp,2));
				temp = plot(plot_here,NaN,NaN);
